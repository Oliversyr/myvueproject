<template>
    <div class="shop-aisle flex-col overflow-y">
        <div class="shop-aisle-header" style="height: 36px; margin-bottom: 15px; border-bottom: 1px solid #ccc;">
            <el-tag type="primary">{{shopInfo.ename}}</el-tag>
            <el-tag type="success">{{posInfo.posName}}号机{{posCounterInfo.devindex}}号柜</el-tag>
            <el-tag type="warning">
                {{
        ...
        <div class="sl-table-col flex-col" ref="shopAisle" v-loading="loading"
        ...
        <el-dialog :visible.sync="adjustPalletGoodsDialogVisible" :close-on-click-modal="false"
                    ...
                    <div v-show="selectGoodsType === 1">
                        ...
                        <el-form-item label="最大存货：" prop="maxQty" style="width: 240px;">
                            <el-input-number size="small" v-model="goodsTemplateInfo.maxQty"
                                             ...
                                             :disabled="posCounterInfo.devType === 3"
...
<script>
    ...
    import {getShopGoodsList, getShopSlaveDetail, setPalletSlave, operateSlaveRowCol} from '@/api/operations/shop/shop'; // 货道设置相关API
    // 组件
    ...
    export default {
        ...
        computed: {
            ...
            maxCol() {// 默认为货柜设备的最大列数
                return this.posCounterInfo.maxCol;
            },
            maxRow() {// 默认为货柜设备的最大行数
                return this.posCounterInfo.maxRow;
            }
        },
        methods: {
            // 获取【店铺】货柜的货道详情
            getShopSlaveDetail() {
                ...
                return getShopSlaveDetail(shopEid, devId).then((res) => {
                    console.log('[店铺管理-设备管理-设备货柜设置]后台返回的货道信息: ', res);
                    let data = res.result;
            ...
            generatorTableData(){
                ...
                console.log('[店铺管理-设备管理-设备货柜设置]货道信息前台展示数据: ', _tableData)
                return _tableData;
            },
            ...
            operateSlaveRowCol(type, rowcolList) {
                let slaveRowColOperRequest = {
                    groupId: this.$parent.shopInfo.groupId || -1,
                    modelId: this.posCounterInfo.modelId || -1,
                    eid: this.$parent.shopInfo.eid || -1,
                    type: type,
                    rowcolList: rowcolList
                }
                return operateSlaveRowCol(slaveRowColOperRequest).then((res) => {
                    console.log('[店铺管理-设备管理-设备货柜设置]货柜行列操作: ', res);
                }).catch((err) => {
                    console.log(err);
                })
            },
            ...
            addSlaveRow() {
                ...
                this.tableData.forEach((item, index) => {
                    ...
                    item.rowColData.forEach((data) => {
                        rowcolList.push({
                            ...
                            counterid: this.posCounterInfo.devindex,
                ...
                var end = new Date();
                console.log('[货道管理-店铺货道详情-FORM-货柜行列设置]生成后台【新增行】计算耗时 =>', (end.getTime() - start.getTime()) + 'ms');
            ...
            delSlaveRow(curItem) {
                this.$confirm('确定删除一行?', '提示', {
                    ...
                    console.log("[店铺管理-设备管理-设备货柜设置]删除一行->删除行详情:", curItem);
                    ...
                    console.log('[店铺管理-设备管理-设备货柜设置]生成后台【删除行】参数: rowcolList =>', rowcolList);

                    // 发送删除行请求
                    this.operateSlaveRowCol(2, rowcolList).then((res) => {
                        console.log('[店铺管理-设备管理-设备货柜设置]删除一行->货柜的货道详情: ', res);
                        this.loading = false;
                        this.$message({
                            message: `删除一行成功`,
                            type: 'success'
                    ...
                    that.tableData.forEach((item, index) => {
                        ...
                        item.rowColData.forEach((data) => {
                            rowcolList.push({
                                ...
                                counterid: that.posCounterInfo.devindex,
                                maxQty: data.chnMaxGoods || 0,
                                weightProp: data.weightProp || 0
                            })
                        })
                    })
                    console.log('[店铺管理-设备管理-设备货柜设置]生成后台【新增、删除列】参数: rowcolList =>', rowcolList);
            ...
            openAdjustPalletGoodsDialog(data, rowIndex, colIndex) {
                console.log('[店铺管理-设备管理-设备货柜设置]打开调整商品弹出框: data =>', data, this.posCounterInfo);
                ...
                this.goodsTemplateInfo.counterId = this.posCounterInfo.devindex;
            ...
            goodsAutoQuery(key){
                if (this.selectGoodsType === 1) {
                    let params = {
                            shopEid: this.$parent.shopInfo.eid,
                            goodsName: key,
                            brandId: -1,
                            catId: -1,
                            status: 1,
                    ...
                    let goodsId = -1,
                        bomGoodsName = key,
                        ename = this.$parent.shopInfo.ename,
                        shopEid = this.$parent.shopInfo.eid,
                        status = 1,
                        page = {
                            pageNum: 1,
            ...
            handleGoodsAutoSelect(sel){
                console.log('[店铺管理-设备管理-设备货柜设置]商品自动补全商品原信息', sel)
            ...
            // 调整商品
            adjustPalletGoods () {
                console.log('[店铺管理-设备管理-设备货柜设置]调整商品的商品模板信息: ', this.goodsTemplateInfo);
                let shopEid = this.$parent.shopInfo.eid,
                    slaveRowCol = {
                        devId: this.posCounterInfo.devId,
                ...
                return setPalletSlave(shopEid, slaveRowCol).then((res) => {
                    console.log('[店铺管理-设备管理-设备货柜设置]调整商品API返回结果: ', res);
                    this.$message({
                        message: `货道商品设置成功`,
                        type: 'success'
                    });
        ...
        mounted () {
            const that = this;
            console.log('shop device aisle mounted', that);
            window.onresize = () => {
                return (() => {
                    that.aisleTempTableWidth = that.$refs.shopAisle.clientWidth;
                })()
            }
        },
        updated: function () {
            this.$nextTick(function () {
                ...
                this.tableData.forEach((item) => {
                    ...
                    doc.controlsSiblingElemDisabledClass(elem);
                })
            })
        },
        mounted() {
            this.posCounterInfo = this.deviceInfo;
            this.aisleTempTableWidth = this.$refs.shopAisle.clientWidth;
            this.aisleTempTableActualWidth = 22 + 70 + 110 * this.maxCol + 200 * 2;
            this.getShopSlaveDetail();
            console.log('[店铺管理-设备管理-设备货柜设置]流入组件的店铺信息、售卖机信息、货柜信息: ', this.shopInfo, this.posInfo, this.deviceInfo);
        },
        directives: {
            disabled: {
                // 指令的定义
