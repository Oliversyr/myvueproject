    ...
    <div class="sl-page-container flex-col report">
        <div class="slic-head-panel">
            ...
            <div style="float: right;">
                <span class="sl-content-handle-left">
                    <el-button-group>
                        <excel-import-export v-if="myModulePower.export" ref="expImp" :fileName="'店铺销售流水'" :header="gridCol" @getResultArr="importData" @goToExp="doExport" :exportData="expData" :isImport="false"></excel-import-export>
                    </el-button-group>
                </span>
            </div>
        ...
        <div class="slic-content" v-loading="loading">
            <div class="slic-content-info">
                <div class="sl-content-handle" style="text-align: left;height: 30px;">
                    <span class="sl-content-handle-right ">
                        <div class="animated fadeIn sl-table-header sl-amount-panel">
                            <div class="sl-amount-panel-item" > 【{{superiorParam.eid}}】 {{superiorParam.ename}}</div> 
                ...
                <div class="sl-grid-container mt" >
                    <sl-grid :gridCol="gridCol" :gridValue="gridVal" :gridConfig="gridCfg" @rowSelected="doSelected" @pageSizeChange="handleSizeChange" @currentPageChange="handleCurrentChange">
                        <template slot="operate" scope="prop">
                            <span slot="tip-content" class="icon-help-wrap pst-rl" title="店铺商品流水" @click="goToReportSaledetail(prop.scope.row)">
                                <icon-svg icon-class="store_goods_flow"></icon-svg>
                            </span>
                        </template>
                    </sl-grid>
                </div>
...
import { getShopSalesummaryList } from '@/api/operations/report/report';
...
export default {
    ...
    mounted: function() {
        this.$nextTick(function() {
            // 获取组织机构列表
            this.gridCol = [
                { field: 'sheetId', name: '销售单号', minWidth:'100', fixed: true },
                { field: 'saleMoney', name: '销售金额', minWidth: '100'  , align: 'right'},
                { field: 'goodsCount', name: '商品数', minWidth: '100'  , align: 'right'},
                { field: 'payTypeStr', name: '支付方式', minWidth: '100' },
    ...
    methods: {
        getShopSalesummaryList(params) {
            this.loading = true;
            getShopSalesummaryList(params).then((res) => {
                this.loading = false;
                if (res.retCode === 'SUCCESS') {
                    this.gridVal.rows = res.result;
                    this.gridVal.rows.map((row)=>{
                        row.eidStr = `【${row.eid}】 ${row.ename}`;
                        row.payTypeStr = utils.filter.payType(row.payType);
                    });
                    this.gridVal.footer = {
                        pageNum: res.pageNum,
        ...
        goToReportSaledetail(item) {
            ...
            this.$router.push({path: '/operations/reportSaledetail', query: param });
        },
        // 查询
        doSearch() {
            setTimeout(()=> {
                ...
                this.getShopSalesummaryList(params);
            }, 100);
        },
        // 重置
        doReset() {
