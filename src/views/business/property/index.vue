<template>
    <div class="sl-page-container flex-col propertyIndex" v-loading="load1">
        <div class="slic-head-panel">
            <div>
                    ...
                    <span slot="tip-content" class="icon-help-wrap pst-rl">
                        ...
                        <div class="icon-help-alert animated zoomIn">
                            <div class="icon-help-alert-til">
                                <span class="left">【业务应用-物业协议管理】</span>
                            </div>
                            <div class="icon-help-alert-tcon">
                                <p>1. 管理物业公司及物业协议；</p>
                                <p>2. 能够查看物业公司的协议签署情况；</p>
                                <p>3. 查看协议详情；</p>
                                <p>4. 新增、修改、终止协议。</p>
                            </div>
                        </div>
                ...
                <div style="float: right">
                    <!--<span style="color: #28e;" v-if="propertyTab === '按物业公司查看'">
                        您还有 <strong style="font-size: 16px;"> {{waitCheck}} </strong> 份物业协议待审批
                        <el-button type="primary" size="small" @click="toCheck()">去审批</el-button>
                    </span>-->

                    <!--<span style="color: #ff4949;" v-if="propertyTab === '按物业协议查看'">
                        有 <strong style="font-size: 16px;"> {{out}} </strong> 份物业协议即将或已过期
                        <el-button type="primary" size="small" @click="toLook()">去看看</el-button>
                    </span>-->
					
					<el-badge :value="waitCheck" :max="99" :title="'您还有' + waitCheck + '份物业协议待审批'" v-if="propertyTab === '按物业公司查看'">
	                    <el-button type="primary" size="small" @click="toCheck()" 
	                               >去审批
	                    </el-button>
	                </el-badge>  
	                
	                <el-badge :value="out" :max="99" :title="'您还有' + out + '份物业协议即将或已过期'"  v-if="propertyTab === '按物业协议查看'" >
	                    <el-button type="primary" size="small" @click="toLook()"
	                               >去看看
	                    </el-button>
	                </el-badge>

                    <el-button-group class="ml-small" v-if="propertyTab === '按物业协议查看'">
            ...
            <div class="sl-search-panel">
                    ...
                    <div class="slic-input-group sl-search-item mr" style="width: 335px;">
                        <!--<sl-area-select :areaId="" @getAreaId="getArea" :showPrepend="true"
                                        :showModal="false">
                     ...
                     <div class="slic-input-group sl-search-item mr" style="width: 300px;" v-if="propertyTab === '按物业协议查看'">
        ...
        <div class="slic-content-info-tab mt-xsmall" >
            <el-radio-group v-model="propertyTab" @change="changeTab">
                <el-radio-button label="按物业公司查看"></el-radio-button>
                <el-radio-button label="按物业协议查看"></el-radio-button>
            </el-radio-group>
        </div>

        <div class="slic-content">
            <div class="sl-content-info">
                <div style="flex: 1;height: 100%;" v-show="propertyTab === '按物业公司查看' ">
                    ...
                    <sl-grid :gridCol="cgridCol" :gridValue="cgridVal" :gridConfig="cgridCfg" v-loading="load2"
                             ...
                             @currentPageChange="handleCCurrentChange" v-if="propertyTab === '按物业公司查看'">
                ...
                <div style="flex: 1;height: 100%;" v-show="propertyTab === '按物业协议查看' ">
                    ...
                    <sl-grid :gridCol="pgridCol" :gridValue="pgridVal" :gridConfig="pgridCfg" v-loading="load2"
                             ...
                             @currentPageChange="handlePCurrentChange" v-if="propertyTab === '按物业协议查看'">
                        <template slot="operate" scope="prop" >
        ...
        <el-dialog title="添加物业协议" :visible.sync="showAddProperty" class="addProperty" :close-on-click-modal="false">
            <span v-loading="load3">
                <el-form :model="addProperty" :rules="rules1" ref="addForm" label-width="95px">
                    <el-row :gutter="15" >
	                <el-col :span="24">
                        <el-form-item label="物业公司：" prop="eid">
	            ...
	            <el-row :gutter="15" >
	                <el-col :span="24">
                        <el-form-item label="店铺：" prop="coeid">
                            <org-select :searchParam="shopSelectParam" @doSelectOrg="getShop" selMode="SINGLE" :proType="1" v-if="showAddProperty"></org-select>
                        </el-form-item>
	                </el-col>
	            ...
	            <el-row :gutter="15" >
	                ...
	                <el-col :span="12">
                        <el-form-item label="附件：" >
                            <!--<el-input size="small" v-model="myfilename"  disabled></el-input>-->
        ...
        <el-dialog title="续签物业协议" :visible.sync="showRenew" :close-on-click-modal="false" class="small-property">
            <el-form :model="reProperty" :rules="rules2" ref="renewForm" label-width="95px">
                ...
                <el-row :gutter="15">
                    ...
                    <el-col :span="24">
                        <el-form-item label="物业签约人：" prop="contact">
...
<script>
    ...
    import {Pl_httpDownFile, BrowserSaveAs} from '../common/utils.js'
...
//                    console.log('12345555555555555==', !(/^[0-9]{11}$/).test(value));
            ...
            return {
                ...
                rules1: {
                    eid: [
                        {required: true, message: '请选择物业公司店铺', trigger: 'change', type: 'number'},
...
//                this.userInfo = JSON.parse(sessionStorage.getItem('userInfo'));
                ...
                this.cgridCol = [
                    {field: 'eid', name: '合作伙伴编码', width: '110', fixed: true, template: 'eid'},
...
//                    this.$message(res.retMsg);
            ...
            // 查看物业公司详情
            toPartnerDetail(obj) {
                this.$router.push({path: '/basic/partnerDetail', query: {coEid: obj.eid, from: 'property'}});
            },
            // 获取按协议查看列表
            getPropertyProtocol() {
...
//                this.addProperty = {};
            ...
            // 续签协议弹窗--确认续签
            doReSure() {
                console.log(this.reProperty.mydate);
//                this.reProperty.mydate = typeof this.reProperty.mydate === 'string' ? this.reProperty.mydate : this.dateFormat(this.reProperty.mydate, 'yyyy-MM-dd')
...
//                    this.$message(res.retMsg);
            ...
            doExport(str) {
                if (str === 'current') {
                    ...
                    if(this.selectedPGridData.length === 0) {
...
//                        this.$message(res.retMsg);
            ...
            finishFun(data) {
                ...
                if(data.retCode==0){
                    BrowserSaveAs(data.blobData, '附件/currentRenew.oldFileName');
                }else{
            ...
            doDownload() {
                ...
                let config = {
                    url: this.userInfo.fileServerAddr + this.currentRenew.filename,
                    progress: {status: 1},
                    finishFun: this.finishFun
                };

                Pl_httpDownFile(config);

                /*let a = document.createElement('a');
                a.href = this.userInfo.fileServerAddr + this.currentRenew.filename;
                console.log(this.currentRenew.oldFileName);
                    ...
                    reader.onload = function (e) {
                        ...
                        axios.post(upLoadUrl, paramStr, {
                            ...
                            if(str === 'new') {
                                ...
                                console.log('_this.addProperty==', _this.addProperty);
                            } else if(str === 'refresh') {
